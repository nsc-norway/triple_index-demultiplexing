...
for item in os.listdir('20_demultiplex/20_header/common/'):
    if SAMPLE_NAME + '_' in item:
        ...
        print >> out_file, 'grep -A3 -Fxf ' + item_path + ' ' + read1 + ' | grep -v "^\-\-$" > ' + out_path + '_R1.fq'
        print >> out_file, 'grep -A3 -Fxf <(sed "s/1:N:0/2:N:0/g" ' + item_path + ') ' + read2 + ' | grep -v "^\-\-$" > ' + out_path + '_R2.fq'
        print >> out_file, ''
